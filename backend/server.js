...
app.get('/', (req, res) => {
  res.json({
    status:     'ok',
    message:    '🤖 FaizUpyZone Chatbot Backend!',
...
app.post('/chat', async (req, res) => {
  if (GROQ_KEYS.length === 0) {
    return res.status(500).json({ error: 'No GROQ keys set. Add GROQ_KEY_1 to GROQ_KEY_7 in .env file.' });
  }
...
app.listen(PORT, () => {
  console.log('\n🚀 FaizUpyZone Chatbot Backend on http://localhost:' + PORT);
