...
//
// Naya behavior:
//   aiDone = false → overlay ruka rehta hai, AI spinner dikhta hai
//   aiDone = true  → 6 sec countdown shuru → onDone() call
//
// Props:
//   isPassed  — boolean
//   score     — number
//   testType  — string ('basic' | 'advanced' | 'pro' | 'neet')
//   aiDone    — boolean (true = AI complete ya fail ho gaya)
//   onDone()  — redirect callback
// ==========================================

export default function SubmissionOverlay({ isPassed, score, testType, aiDone, onDone }) {
  ...
  // ── Rotating messages while AI is working ─────────────
  const waitMsgs = [
    '🧠 AI is analyzing your performance...',
    '📊 Building your personalized report...',
    '🎯 Identifying your weak areas...',
    '💡 Preparing improvement tips...',
    '📝 Finalizing your AI report...',
  ];

  // ── Rotate message every 2.2s while AI not done ───────
  useEffect(() => {
    if (aiDone) return;
    const t = setInterval(() => setAiMsgIdx(p => (p + 1) % waitMsgs.length), 2200);
    return () => clearInterval(t);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [aiDone]);

  // ── Countdown starts ONLY after aiDone = true ─────────
  useEffect(() => {
    if (!aiDone) return;
    setCountdown(6);
    const t = setInterval(() => {
  ...
  const progressPct = aiDone
    ? ((6 - Math.max(0, countdown)) / 6) * 100
    : 0;
  ...
  const dashOffset   = aiDone
    ? circumference - (Math.max(0, countdown) / 6) * circumference
    : circumference;
  const timerColor = isPassed ? '#10b981' : '#6366f1';
  const trackColor = isPassed ? 'rgba(16,185,129,0.12)' : 'rgba(99,102,241,0.12)';

  const getBottomMsg = () => {
    if (!aiDone) return '⏳ Please wait — your AI performance report is being generated...';
    ...
    return isPassed
      ? '🎓 Certificate issued! Your AI Report is ready — check the Results tab.'
      : '💪 Score 55%+ in your next attempt to earn a Certificate!';
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{
            ...
            {isPassed
              ? (isNeet ? 'NEET Passed' : 'Test Passed')
              : 'Test Submitted'}
          </span>
        </div>
        ...
        <p style={{
          ...
          {aiDone
            ...
            : 'Please wait while we prepare your AI report...'}
        </p>

        {/* Rotating message */}
        <div key={aiDone ? 'done' : aiMsgIdx} style={{
          minHeight:  '1.8rem',
          ...
          {aiDone
            ? (countdown <= 2 ? '👋 See you soon!' : '✅ Report ready! Opening Results tab...')
            : waitMsgs[aiMsgIdx]}
        </div>
          ...
          <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
            ...
            <div style={{
              ...
              {aiDone
                ...
                : <span style={{ animation: 'dotBlink 1s infinite', fontSize: '0.85rem' }}>⏳</span>
              }
            </div>
          </div>
          ...
          <div style={{ textAlign: 'left' }}>
            ...
            <div style={{ fontSize: '0.75rem', color: '#94a3b8', fontWeight: '500' }}>
              {aiDone
                ...
                : 'This may take a few seconds — please stay'}
            </div>
          </div>
        </div>
