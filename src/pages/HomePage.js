...
/* ─────────────────────────────────────────
   STUDENT REVIEWS
   ✅ FIX: Any logged-in user can add reviews
   ✅ FIX: Only admin can delete
───────────────────────────────────────── */
...
function StudentReviews({ isDark, isMobile, isAdmin, user }) {
  ...
  // ✅ Any logged-in user can add a review
  const canUserAddReview = !!user;
  ...
  if (!loading && reviews.length === 0 && !canUserAddReview) return null;
  ...
  return (
    <section ref={ref} style={{ padding: isMobile ? '0 16px 40px' : '0 24px 64px', maxWidth:'900px', margin:'0 auto', position:'relative' }}>
      ...
      {loading ? (
        ...
        canUserAddReview && <div style={{ textAlign:'center', padding:'40px', background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(99,102,241,0.04)', borderRadius:'20px', border: isDark ? '1px dashed rgba(255,255,255,0.1)' : '1px dashed rgba(99,102,241,0.2)', color: isDark ? '#475569' : '#94a3b8', fontSize:'0.9rem' }}>No reviews yet. Be the first to review! 👇</div>
      ) : (
      ...
      {/* ✅ Write Review button for any logged-in user */}
      {canUserAddReview && canAddMore && (
        <div style={{ textAlign:'center', marginTop:'24px' }}>
          <button onClick={() => setShowAddForm(!showAddForm)} style={{ background: showAddForm ? 'transparent' : 'linear-gradient(135deg, #6366f1, #ec4899)', border: showAddForm ? isDark ? '2px solid rgba(255,255,255,0.15)' : '2px solid #e2e8f0' : 'none', color: showAddForm ? isDark ? '#94a3b8' : '#64748b' : '#fff', padding:'10px 28px', borderRadius:'50px', fontWeight:'700', fontSize:'0.88rem', cursor:'pointer', boxShadow: showAddForm ? 'none' : '0 6px 20px rgba(99,102,241,0.35)', transition:'all 0.25s ease' }}>
            {showAddForm ? '✕ Cancel' : '✍️ Write a Review'}
          </button>
        </div>
      )}

      {/* Login hint for logged-out users */}
      {!user && reviews.length > 0 && (
        <div style={{ textAlign:'center', marginTop:'16px' }}>
          <span style={{ fontSize:'0.8rem', color: isDark ? '#475569' : '#94a3b8', fontWeight:'600' }}>🔐 Login to write your own review</span>
        </div>
      )}
      ...
      {canUserAddReview && showAddForm && canAddMore && (
        <AddReviewForm isDark={isDark} isMobile={isMobile} user={user} onSave={async (data) => {
...
function AddReviewForm({ isDark, isMobile, user, onSave, onCancel }) {
  ...
  return (
    <div style={{ marginTop:'24px', background: isDark ? 'linear-gradient(135deg, rgba(15,23,42,0.8), rgba(30,27,75,0.5))' : 'rgba(255,255,255,0.88)', backdropFilter: isMobile ? 'none' : 'blur(16px)', WebkitBackdropFilter: isMobile ? 'none' : 'blur(16px)', border: isDark ? '1px solid rgba(99,102,241,0.2)' : '1px solid rgba(99,102,241,0.18)', borderRadius:'22px', padding: isMobile ? '20px 16px' : '28px 24px', boxShadow: isDark ? '0 8px 32px rgba(0,0,0,0.3)' : '0 8px 32px rgba(99,102,241,0.08)' }}>
      ...
      <h3 style={{ fontSize:'1rem', fontWeight:'900', color: isDark ? '#e2e8f0' : '#1e293b', margin:'0 0 18px' }}>✍️ Write Your Review</h3>
      <div style={{ marginBottom:'14px' }}>
        <label style={{ display:'block', fontSize:'0.8rem', fontWeight:'700', color: isDark ? '#94a3b8' : '#64748b', marginBottom:'8px' }}>Your Photo (Optional)</label>
      ...
      <textarea placeholder="Share your experience *" value={form.text} onChange={handle('text')} rows={3} style={{ ...inputStyle, resize:'vertical', marginBottom:'12px' }} />
      ...
      <div style={{ display:'flex', gap:'10px' }}>
        <button onClick={handleSave} disabled={saving} style={{ flex:1, padding:'12px', background: saving ? 'rgba(99,102,241,0.5)' : 'linear-gradient(135deg, #6366f1, #ec4899)', border:'none', borderRadius:'12px', color:'#fff', fontWeight:'800', fontSize:'0.9rem', cursor: saving ? 'not-allowed' : 'pointer', boxShadow: saving ? 'none' : '0 6px 18px rgba(99,102,241,0.35)' }}>
          {uploading ? '📤 Uploading photo...' : saving ? '⏳ Saving...' : '✅ Submit Review'}
        </button>
...
/* ─────────────────────────────────────────
   ✅ PREMIUM VERTICAL TIMELINE — 2026
   Original colors · Single connected line
   Zigzag cards on desktop · Left on mobile
───────────────────────────────────────── */
function PremiumTimeline({ isDark, isMobile }) {
  const [ref, visible] = useScrollReveal(0.06);

  const events = [
    { date:'1 Jan 2026',  title:'The Idea',       desc:'PySkill was born — a vision to give students quality Python study material at affordable prices.',    icon:'💡', tag:'ORIGIN STORY', color:'#a78bfa', glow:'rgba(167,139,250,0.45)' },
    { date:'10 Jan 2026', title:'Work Begins',     desc:'Development kicked off. Notes curated, questions filtered, platform designed from scratch.',          icon:'⚡', tag:'DEVELOPMENT',  color:'#6366f1', glow:'rgba(99,102,241,0.45)'  },
    { date:'15 Feb 2026', title:'Website Live 🚀', desc:'PySkill officially launched! First students enrolled, first certificates issued.',                    icon:'🚀', tag:'LAUNCH DAY',   color:'#ec4899', glow:'rgba(236,72,153,0.45)'  },
  ];

  // dot size constants
  const DOT = isMobile ? 48 : 52;
  const HALF = DOT / 2;

  return (
    <section ref={ref} style={{ padding: isMobile ? '8px 16px 64px' : '0 24px 80px', maxWidth:'900px', margin:'0 auto' }}>

      {/* Header */}
      <div style={{ textAlign:'center', marginBottom: isMobile ? '36px' : '56px' }}>
        <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', background: isDark ? 'rgba(99,102,241,0.12)' : 'rgba(99,102,241,0.07)', border:`1px solid ${isDark?'rgba(99,102,241,0.3)':'rgba(99,102,241,0.2)'}`, borderRadius:'40px', padding:'6px 16px', marginBottom:'14px', opacity: visible?1:0, transition:'opacity 0.6s ease' }}>
        ...
        <h2 style={{ fontSize: isMobile?'1.65rem':'2.5rem', fontWeight:'900', background:'linear-gradient(135deg, #1e40af, #6366f1, #ec4899)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', margin:'0 0 8px', letterSpacing:'-0.02em', opacity: visible?1:0, transform: visible?'translateY(0)':'translateY(22px)', transition:'opacity 0.7s ease 0.1s, transform 0.7s ease 0.1s' }}>From Idea to Reality</h2>
        <p style={{ color: isDark?'#64748b':'#94a3b8', fontSize: isMobile?'0.82rem':'0.92rem', fontWeight:'500', margin:0, opacity: visible?1:0, transition:'opacity 0.7s ease 0.2s' }}>Every milestone that brought PySkill to life</p>
      </div>

      {/* Timeline wrapper */}
      <div style={{ position:'relative' }}>

        {/* ✅ Single vertical line — perfectly centered on dots */}
        <div style={{
          position:'absolute',
          left: isMobile ? `${HALF - 1}px` : '50%',
          transform: isMobile ? 'none' : 'translateX(-50%)',
          top: `${HALF}px`,
          // subtract 1 dot-height from bottom so line ends at last dot center
          bottom: `${HALF}px`,
          width:'2px',
          background: isDark
            ? 'linear-gradient(180deg, rgba(167,139,250,0.8) 0%, rgba(99,102,241,0.9) 50%, rgba(236,72,153,0.8) 100%)'
            : 'linear-gradient(180deg, rgba(167,139,250,0.5) 0%, rgba(99,102,241,0.6) 50%, rgba(236,72,153,0.5) 100%)',
          opacity: visible ? 1 : 0,
        ...
        {events.map((evt, i) => {
          ...
          const delay  = `${0.32 + i * 0.2}s`;
          return (
            <div key={i} style={{
              display:'flex',
              flexDirection: isMobile ? 'row' : isLeft ? 'row' : 'row-reverse',
              alignItems:'flex-start',
              marginBottom: i < events.length - 1 ? (isMobile ? '28px' : '44px') : 0,
              position:'relative',
              opacity: visible ? 1 : 0,
              transform: visible ? 'translateY(0)' : 'translateY(28px)',
              transition:`opacity 0.6s ease ${delay}, transform 0.65s cubic-bezier(0.34,1.2,0.64,1) ${delay}`,
            }}>
              ...
              <div style={{
                ...
                // Desktop: pull left/right by half to center on the line
                margin: isMobile ? `4px 16px 0 0` : `10px ${-HALF}px 0`,
                zIndex:2, position:'relative',
                background:`linear-gradient(135deg, ${evt.color}, ${i===2?'#f472b6':'#a78bfa'})`,
                borderRadius:'50%',
                display:'flex', alignItems:'center', justifyContent:'center',
                fontSize: isMobile ? '1.3rem' : '1.45rem',
                boxShadow:`0 0 0 ${isMobile?4:5}px ${isDark?'#0f172a':'#eef1ff'}, 0 0 0 ${isMobile?6:7}px ${evt.color}44, 0 8px 28px ${evt.glow}`,
                animation: visible ? `tlDotPulse 2.8s ease-in-out ${delay} infinite` : 'none',
              }}>{evt.icon}</div>

              {/* Card */}
              <div style={{ flex:1, maxWidth: isMobile ? `calc(100% - ${DOT+16}px)` : '44%' }}>
                <TimelineCard evt={evt} isDark={isDark} isMobile={isMobile} />
              </div>
...
function TimelineCard({ evt, isDark, isMobile }) {
  const [hovered, setHovered] = useState(false);
  return (
    <div
      ...
      style={{
        ...
        background: isDark
          ? 'linear-gradient(135deg, rgba(15,23,42,0.85), rgba(30,27,75,0.62))'
          : 'linear-gradient(135deg, rgba(255,255,255,0.93), rgba(240,244,255,0.82))',
        backdropFilter: isMobile ? 'none' : 'blur(18px)',
        WebkitBackdropFilter: isMobile ? 'none' : 'blur(18px)',
        border: hovered ? `1.5px solid ${evt.color}88` : isDark ? '1.5px solid rgba(255,255,255,0.09)' : '1.5px solid rgba(255,255,255,0.98)',
        borderRadius:'22px',
        padding: isMobile ? '18px 16px' : '22px 24px',
        boxShadow: hovered
          ? `0 18px 50px ${evt.glow}, inset 0 1px 0 rgba(255,255,255,0.12)`
          : isDark ? '0 6px 28px rgba(0,0,0,0.38)' : '0 6px 28px rgba(99,102,241,0.09)',
        transform: hovered ? 'translateY(-4px)' : 'translateY(0)',
        transition:'all 0.3s cubic-bezier(0.34,1.2,0.64,1)',
      }}
    >
      {/* Top accent line */}
      <div style={{ position:'absolute', top:0, left:0, right:0, height:'2.5px', background:`linear-gradient(90deg, transparent, ${evt.color}, ${evt.color}88, transparent)`, borderRadius:'22px 22px 0 0' }} />
      {/* Glow blob */}
      <div style={{ position:'absolute', top:'-18px', right:'-18px', width:'72px', height:'72px', background:`radial-gradient(circle, ${evt.color}22 0%, transparent 70%)`, borderRadius:'50%', pointerEvents:'none' }} />

      {/* Tag + Date */}
      <div style={{ display:'inline-flex', alignItems:'center', gap:'6px', background:`${evt.color}18`, border:`1px solid ${evt.color}38`, borderRadius:'30px', padding:'4px 12px', marginBottom:'10px' }}>
        <span style={{ width:5, height:5, borderRadius:'50%', background:evt.color, display:'inline-block', animation:'liveDot 1.6s ease-in-out infinite' }} />
        <span style={{ fontSize:'0.65rem', fontWeight:'900', color:evt.color, letterSpacing:'0.1em' }}>{evt.tag}</span>
        <span style={{ width:1, height:10, background:`${evt.color}40`, display:'inline-block' }} />
        <span style={{ fontSize:'0.65rem', fontWeight:'700', color: isDark?'#64748b':'#94a3b8' }}>{evt.date}</span>
      </div>

      <h3 style={{ fontSize: isMobile?'1rem':'1.1rem', fontWeight:'900', color: isDark?'#e2e8f0':'#0f172a', margin:'0 0 8px', letterSpacing:'-0.01em' }}>{evt.title}</h3>
      <p style={{ fontSize: isMobile?'0.78rem':'0.84rem', color: isDark?'#94a3b8':'#475569', margin:0, lineHeight:1.65 }}>{evt.desc}</p>
    </div>
  );
}
...
function HomePage({ setCurrentPage }) {
  ...
  return (
    <div style={{ paddingTop: isMobile?'62px':'70px', minHeight:'100vh', overflowX:'hidden', willChange:'scroll-position', WebkitOverflowScrolling:'touch', contain:'paint' }}>
      ...
      <section ref={mockRef} style={{ padding: isMobile?'0 16px 32px':'0 24px 48px', maxWidth:'1000px', margin:'0 auto', opacity: mockVis?1:0, transform: mockVis?'translateY(0)':'translateY(24px)', transition:'opacity 0.55s ease, transform 0.55s ease' }}>
        <div style={{ background: isDark?'rgba(15,23,42,0.6)':'rgba(240,244,255,0.7)', border: isDark?'1.5px solid rgba(99,102,241,0.25)':'1.5px solid rgba(99,102,241,0.18)', borderRadius: isMobile?'20px':'26px', padding: isMobile?'20px 16px':'28px 32px', position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap: isMobile?'10px':'14px', marginBottom: isMobile?'14px':'18px' }}>
            {[
              ...
              <div key={i} onClick={() => setCurrentPage('mocktests')} style={{ background: isDark?`${lvl.color}18`:`${lvl.color}12`, border:`1.5px solid ${lvl.color}${isDark?'40':'28'}`, borderRadius: isMobile?'14px':'16px', padding: isMobile?'14px 10px':'18px 14px', textAlign:'center', cursor:'pointer', transition:'all 0.25s ease' }}
                ...
                <div style={{ fontSize: isMobile?'0.68rem':'0.75rem', fontWeight:'700', color: isDark?'#94a3b8':'#64748b', lineHeight:1.6 }}>{lvl.q}<br/>{lvl.t}</div>
              </div>
            ))}
          </div>
          <div style={{ display:'flex', flexWrap:'wrap', gap:'6px', justifyContent:'center' }}>
            {['🔒 Anti-Cheat', '🚫 Tab Switch Detection', '🖥️ Fullscreen Mode', '🏆 Certificate 55%+'].map((item, i) => (
              <span key={i} style={{ fontSize: isMobile?'0.65rem':'0.72rem', fontWeight:'700', color: isDark?'#64748b':'#94a3b8', background: isDark?'rgba(255,255,255,0.05)':'rgba(0,0,0,0.04)', border: isDark?'1px solid rgba(255,255,255,0.08)':'1px solid rgba(0,0,0,0.07)', borderRadius:'20px', padding: isMobile?'3px 9px':'4px 11px' }}>{item}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      {/* ═══ STUDENT REVIEWS — user prop passed ═══ */}
      <StudentReviews isDark={isDark} isMobile={isMobile} isAdmin={isAdmin} user={user} />

      {/* ═══ ✅ PREMIUM VERTICAL TIMELINE ═══ */}
      <PremiumTimeline isDark={isDark} isMobile={isMobile} />
...
function ActionCard({ card, isDark, isMobile, onClick }) {
  ...
  return (
    <button onClick={onClick}
      ...
      style={{
        background: hovered ? isDark?'rgba(255,255,255,0.09)':'rgba(255,255,255,0.98)' : isDark?'rgba(255,255,255,0.05)':'rgba(255,255,255,0.82)',
        backdropFilter: isMobile?'none':'blur(12px)', WebkitBackdropFilter: isMobile?'none':'blur(12px)',
        border: hovered ? `1.5px solid ${card.color}55` : `1.5px solid ${isDark?'rgba(255,255,255,0.1)':'rgba(99,102,241,0.18)'}`,
        borderRadius: isMobile?'18px':'20px', padding: isMobile?'16px 8px':'20px 12px', cursor:'pointer',
        display:'flex', flexDirection:'column', alignItems:'center', gap: isMobile?'8px':'10px',
        position:'relative', overflow:'hidden',
        transform: pressed?'scale(0.93)':hovered?'translateY(-4px)':'scale(1)',
        transition:'transform 0.15s ease, box-shadow 0.2s ease, border-color 0.2s ease, background 0.2s ease',
        boxShadow: hovered ? `0 10px 28px ${card.glow}` : isDark?'0 2px 12px rgba(0,0,0,0.25)':'0 2px 12px rgba(99,102,241,0.07)',
        animation:`cardRise 0.45s ease ${card.delay} both`, outline:'none',
      }}>
      ...
      <div style={{ width: isMobile?'40px':'46px', height: isMobile?'40px':'46px', background:card.gradient, borderRadius:'13px', display:'flex', alignItems:'center', justifyContent:'center', fontSize: isMobile?'1.2rem':'1.4rem', boxShadow:`0 4px 14px ${card.glow}`, position:'relative', zIndex:1, transform: hovered?'scale(1.12)':'scale(1)', transition:'transform 0.2s ease' }}>{card.icon}</div>
      <span style={{ fontSize: isMobile?'0.72rem':'0.82rem', fontWeight:'800', color: hovered?card.color:isDark?'#e2e8f0':'#1e293b', position:'relative', zIndex:1, transition:'color 0.2s ease' }}>{card.label}</span>
      {hovered && <span style={{ fontSize:'0.6rem', color:card.color, fontWeight:'700', opacity:0.8, position:'relative', zIndex:1, animation:'fadeInUp 0.2s ease' }}>tap →</span>}
    </button>
  );
}
